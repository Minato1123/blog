...
* [useVModels](https://vueuse.org/core/usevmodels/#usevmodels)
  > props `v-model` 綁定的簡寫。  
  > 與 `toRefs(props)` 相似，但變化同樣會觸發 `emit`。  
  :::tip 其他
...
* `template`
  ...
  > ```
  > 4. `grid-template-area`  <span class="span-mb"></span>
...
* `column / row`
  ...
  > ```
  > * `grid-column-gap`
  > * `grid-row-gap`  <span class="span-mb"></span>
  > 增加線的寬度以調整網格的間距。
  > * `grid-gap`
  > ```css
...
* [Astro](https://astro.build/)
  ...
  > 1. 自帶元件框架：  Astro 為 React、Vue、Svelte 和 Tailwind CSS 等前端工具提供一級支援，透過 astro add 命令即可新增使用。
  > 2. 支援靜態頁面生成 (SSG) 和伺服器端渲染 (SSR)，可以按需渲染內容。
  > 3. 開發者體驗出眾：  Astro 支援所有喜愛的開發者工具和功能，如 TypeScript、NPM 包、Scoped CSS、CSS Modules、Sass、Tailwind、Markdown、MDX。
  > 4. 按需元件：  Astro 支援通過水化元件按需載入 JavaScript。因此，如果該特定元件不可見，它不會載入相關的 JavaScript。
  > 5. 100% 靜態 HTML，無 JavaScript 執行時環境：當構建 Astro 時，它將刪除所有 JavaScript，並將整個頁面渲染為靜態 HTML 頁面。
  > 6. SEO 友好：  使用 Astro，可以啟用自動網站地圖、RSS 訂閱、分頁和收藏。
  > 7. 基於檔案的路由：  像 `Next.js` 一樣，Astro 有一個基於檔案的路由機制，所有在 `/pages` 中的東西 Astro 都會將目錄轉化為路由。
  > 
