...
* `window.onunload`
  ...
  > 我們可以在那裡做一些**不涉及延遲**的操作，例如關閉相關的彈出視窗。
...
* `document.readyState`
  > 1. `loading`：文件正在被載入。
  > 2. `interactive`：文件被全部讀取；與 `DOMContentLoaded` 幾乎同時發生，但是在 `DOMContentLoaded` **之前**發生。
  > 3. `complete`：檔案被全部讀取，並且所有資源（例如圖片等）都已載入完成；與 `window.onload` 幾乎同時發生，但是在 `window.onload` **之前**發生。
* `readystatechange` 事件
