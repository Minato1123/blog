...
* [CodeGPT](https://www.codegpt.co/)
  > 這個擴充套件允許我們通過官方 OpenAI API 在 VSCode 內部使用 GPT-3。
  >
  > [⋯ Reference](https://news.sangniao.com/p/2371508219)

* [fast-json-stringify](https://github.com/fastify/fast-json-stringify)
  > 需要 JSON Schema Draft 7 輸入來生成快速 `stringify` 函數。<br>
  > 主要用來給我們生成一個客製化的 `stringify` 函數，從而來提升 `stringify` 的效能。
  >
...
Haml 為 HTML 中提供了一些動態內容的靈活性，類似其他 Web 語言和模板系統，Haml 也嵌入一些在執行時執行的代碼並生成 HTML 代碼，從而提供一些動態內容。為執行 Haml 程式碼，檔案需要採用 `.haml` 副檔名。<br>
:::
...
* [Fresh](https://fresh.deno.dev/)
  ...
  > * 缺點<br>
  > 1. 僅支持 Preact 框架，不支持 React，這一點是比較致命的。
...
* Phoenix Liveview
  > 是一個用於構建單頁面應用程式 (SPAs) 的庫，它是 Phoenix Framework 的一部分。<br>
  ...
  > 首先，它使用了伺服器端渲染，這意味著伺服器端可以生成 HTML，而不是客戶端使用 JavaScript 渲染。這有助於提高應用程式的效能，因為伺服器端渲染可以更快地載入頁面。<br>
...
* [GraphQL](https://graphql.org/)
  ...
  > 2. 程式即文檔<br>
  > 前後端溝通成本減少、實現以資料需求驅動 (driven by data requirement) 的設計、建立文檔 (documentation) 的時間成本幾近為 0。
  > 3. 前端控制權提升<br>
