...
* `::before` 與 `::after`
  ...
  > 同時偽元素也會<span class="span-heightlight">繼承</span>原本元素的屬性。<br>
  
  > 一定要具備 content 的屬性，就算是只有 `content: "";` 都可以，因為沒有 content 的偽元素是不會出現在畫面上的，然而 content 是個很特別的屬性，它可以使用 attr 直接獲取內容元素的屬性值 (attribute）。<br>
  > content 內容是可以<span class="span-heightlight">相加</span>的，不過用法不像 JS 使用 + 號來相連，而是直接用一個空白鍵就可以不斷的累加下去。<br>
  
  > 偽元素的內容實際上不存在網頁裡 (如果打開瀏覽器的開發者工具，是看不到內容的)，所以如果在裡頭塞了太多的重要的內容，反而會影響到 SEO 的成效，因此對於使用偽元素的定位，還是當作<span class="span-heightlight">輔助</span>性質會比較恰當。
